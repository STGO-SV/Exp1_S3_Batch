...
class MobileJwtSecurityTests extends JwtTestSupport {
    ...
    @ParameterizedTest @ValueSource(strings = {"WEB", "ATM"})
    ...
    void rejectsInvalidTokens(String scenario) throws Exception {
        mvc.perform(get(URL).header("Authorization", "Bearer " + token("MOBILE", scenario)))
                .andExpect(status().isUnauthorized());
    }
    @Test void rejectsAlteredPayload() throws Exception {
        String jwt = token("MOBILE");
    ...
    @Test void rejectsExternalErrorRequest() throws Exception {
        mvc.perform(get("/error").with(bearer("MOBILE"))).andExpect(status().isForbidden());
    }
}
